...
(function(S,w){typeof exports=="object"&&typeof module<"u"?w(exports):typeof define=="function"&&define.amd?define(["exports"],w):(S=typeof globalThis<"u"?globalThis:S||self,w(S.MSSAFrameKit={}))})(this,function(S){"use strict";function w(t,e,i){const a=document.querySelector("#left-hand"),o=document.querySelector("#right-hand");t==="left"?y(a,e,i):t==="right"?y(o,e,i):t==="both"&&(y(a,e,i),y(o,e,i))}function H(t,e){const i=document.querySelector("#left-hand"),a=document.querySelector("#right-hand");let o=0;e.forEach(n=>{setTimeout(()=>{console.log(`Vibrating ${t} hand for ${n.duration}ms with intensity ${n.intensity}`),t==="left"?y(i,n.duration,n.intensity):t==="right"?y(a,n.duration,n.intensity):t==="both"&&(y(i,n.duration,n.intensity),y(a,n.duration,n.intensity))},o),o+=n.duration})}function y(t,e,i){if(e<=5e3)t.setAttribute("haptics__trigger",`dur: ${e}; force: ${i}`),t.emit("trigger-vibration");else{let n=function(){o>5e3?(t.setAttribute("haptics__trigger",`dur: 5000; force: ${i}`),t.emit("trigger-vibration"),o-=5e3,setTimeout(n,5e3)):(t.setAttribute("haptics__trigger",`dur: ${o}; force: ${i}`),t.emit("trigger-vibration"))},o=e;n()}}const x=Object.freeze(Object.defineProperty({__proto__:null,initVibration:y,triggerHapticPattern:H,triggerHaptics:w},Symbol.toStringTag,{value:"Module"}));AFRAME.registerComponent("arm-swing-movement",{schema:{enabled:{type:"boolean",default:!0},leftController:{type:"selector",default:'[oculus-touch-controls*="hand: left"], [oculus-touch-controls*="hand:left"], [meta-touch-controls*="hand: left"], [meta-touch-controls*="hand:left"], [hand-controls*="hand: left"], [hand-controls*="hand:left"]'},rightController:{type:"selector",default:'[oculus-touch-controls*="hand: right"], [oculus-touch-controls*="hand:right"], [meta-touch-controls*="hand: right"], [meta-touch-controls*="hand:right"], [hand-controls*="hand: right"], [hand-controls*="hand:right"]'},speedFactor:{type:"number",default:1},smoothingTime:{type:"number",default:500},minSpeed:{type:"number",default:null},maxSpeed:{type:"number",default:null},swingTimeout:{type:"number",default:700},avgDirectionSampleInterval:{type:"number",default:100},avgDirectionBufferSize:{type:"number",default:20},reverseButtonEvent:{type:"string",default:""},reverseButtonHand:{type:"string",default:""},debug:{type:"boolean",default:!1},soundEntity:{type:"selector",default:""},soundVolume:{type:"number",default:1},oneStepPlaybackRate:{type:"number",default:1}},init:function(){if(!this.data.enabled)return;this.controllerArrows=[];const t=this.createControllerArrow("left"),e=this.createControllerArrow("right");if(t&&this.controllerArrows.push(t),e&&this.controllerArrows.push(e),this.avgArrow=this.createAvgDirectionArrow(),this.samples=[],this.timeSinceLastSample=0,this.reverseHeld=!1,this.data.reverseButtonEvent){const i=this.data.reverseButtonEvent,a=i.replace(/(?:down|start)$/,n=>n==="down"?"up":"end");let o;this.data.reverseButtonHand==="left"?o=this.data.leftController:this.data.reverseButtonHand==="right"?o=this.data.rightController:o=this.el,o.addEventListener(i,()=>{this.reverseHeld=!0}),o.addEventListener(a,()=>{this.reverseHeld=!1})}this.audioEl=this.data.soundEntity||null,this.audioEl&&(this.audioEl.volume=this.data.soundVolume),this.hands={left:{entity:this.data.leftController,lastZ:null,lastDirection:null,lastSwingTime:null,recentSwings:[]},right:{entity:this.data.rightController,lastZ:null,lastDirection:null,lastSwingTime:null,recentSwings:[]}},this.currentSpeed=0,this.swingDetectThreshold=.01,this.moving=!1},tick:function(t,e){if(!this.data.enabled)return;this.timeSinceLastSample+=e,this.timeSinceLastSample>=this.data.avgDirectionSampleInterval&&(this.timeSinceLastSample-=this.data.avgDirectionSampleInterval,this.updateDirection());for(let d in this.hands){let s=this.hands[d];if(!s.entity)continue;let h=new THREE.Vector3;s.entity.object3D.getWorldPosition(h);let f;if(this.el.avgDirectionVec){let r=new THREE.Vector3;this.el.object3D.getWorldPosition(r),f=h.clone().sub(r).dot(this.el.avgDirectionVec)}else f=this.el.object3D.worldToLocal(h.clone()).z;if(s.lastZ===null){s.lastZ=f;continue}let m=f-s.lastZ,u=s.lastDirection;if(m>this.swingDetectThreshold?u="positive":m<-this.swingDetectThreshold&&(u="negative"),s.lastDirection&&u&&u!==s.lastDirection){if(s.lastSwingTime!==null){let r=t-s.lastSwingTime;r>150&&(s.recentSwings.push(r),s.recentSwings.length>6&&s.recentSwings.shift())}s.lastSwingTime=t}s.lastDirection=u,s.lastZ=f}for(let d in this.hands){let s=this.hands[d];s.lastSwingTime!==null&&t-s.lastSwingTime>this.data.swingTimeout&&(s.recentSwings=[],s.lastSwingTime=null,s.lastDirection=null)}let i=[];for(let d in this.hands)i=i.concat(this.hands[d].recentSwings);const a=i.length;if(i=i.filter(d=>d>0),a<10){let d=10-a;for(let s=0;s<d;s++)i.push(800)}let o=0;o=i.reduce((d,s)=>d+s,0)/i.length;let n=0;const l=1e3/o;o>0&&(n=3.45*l-3.95,n*=this.data.speedFactor,this.data.maxSpeed?n=Math.min(n,this.data.maxSpeed):n=Math.min(n,10*this.data.speedFactor));const c=this.data.minSpeed||.6*this.data.speedFactor;if(n<c?(n=0,this.moving=!1):this.moving=!0,this.currentSpeed+=(n-this.currentSpeed)*(e/this.data.smoothingTime),this.currentSpeed=Math.max(0,this.currentSpeed),this.audioEl&&(this.audioEl.playbackRate=this.data.oneStepPlaybackRate*l,this.moving?this.audioEl.paused&&this.audioEl.play():this.audioEl.paused||this.audioEl.pause()),this.data.debug){const d=i.map(s=>Math.round(s)).join(", ");console.log(`Steps/sec: ${l.toFixed(1)}, Target m/s: ${n.toFixed(1)}, Current m/s: ${this.currentSpeed.toFixed(1)}, avgSwingTime: ${o.toFixed(1)}, recentSwings: [${d}]`)}let E=this.currentSpeed*(e/1e3),b=new THREE.Vector3;this.el.avgDirectionVec?b.copy(this.el.avgDirectionVec).negate():(this.el.object3D.getWorldDirection(b),b.negate());let p=this.el.components["movement-controls"],T=this.el.sceneEl.systems.nav;if(p&&p.data.constrainToNavMesh&&T){let d=this.el.object3D.position.clone(),s=d.clone().add(b.clone().multiplyScalar(E)),h=p.navGroup||T.getGroup(d),f=p.navNode||T.getNode(d,h),m=new THREE.Vector3,u=T.clampStep(d,s,h,f,m);this.el.object3D.position.copy(m),p.navGroup=h,p.navNode=u}else this.el.object3D.position.add(b.multiplyScalar(E))},createControllerArrow:function(t){const e=this.el.querySelector(`#${t}-hand`);if(!e)return null;const i=document.createElement("a-entity");return i.setAttribute("class","controller-arrow"),i.setAttribute("position","0 -0.083 -0.167"),i.setAttribute("rotation","-30 0 0"),i.innerHTML=`
